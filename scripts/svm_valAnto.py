        ...
        for epoch in epoch_pbar:
            ...
            if epoch_avg_loss < best_avg_loss - 1e-4:
                ...
                if wait >= self.patience:
                    epoch_pbar.close()
                    break
            epoch_pbar.set_postfix(avg_loss=f"{epoch_avg_loss:.4f}", wait=wait, val_acc=f"{val_acc:.4f}" if val_acc is not None else "n/a")
            val_str = f", val_acc={val_acc:.4f}" if val_acc is not None else ""
            print(f"    epoch {epoch}: avg_loss={epoch_avg_loss:.4f} (patience wait={wait}, best={best_avg_loss:.4f}{val_str})")
...
def main():
    ...
    SUBFOLDER_DIR = MODELS_DIR / SUBFOLDER
    ...
    class_report = classification_report(y_test, y_pred_final, target_names=ALL_LABELS, digits=4)
    print(f"\nFinal Test Accuracy: {test_acc:.4f}")
    print(class_report)

    print(f"\nTotal Tournament Time: {_format_duration(time.time() - t_start_total)}")

    # Save results and model in models/{N}f only
    results = {
        "features": nf,
    ...
    # Save results JSON
    with open(SUBFOLDER_DIR / "svm_val_results.json", "w") as f:
        json.dump(results, f, indent=2)
    # Save model and label encoder
    import joblib
    joblib.dump(final_clf, SUBFOLDER_DIR / "svm_model.joblib")
    joblib.dump(le, SUBFOLDER_DIR / "svm_label_encoder.joblib")
    print(f"Results and model saved to: {SUBFOLDER_DIR}")
